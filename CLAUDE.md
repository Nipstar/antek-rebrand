...
- Visible order (conversion-first): HERO (benefit H1 incl. town + subhead + Book a free 30-min call + click-to-call `tel:03330389960`) → QuickRecap → PROBLEM → OFFER (3 services as outcomes + single CTA, H2 = "AI automation for [Town] businesses") → TrustBlock → "Our [Town] patch" entity layer → ResourcesCompliance → sibling/geo cards (up to hub + 3 siblings) → FAQ (5 inline `<details>`) → final CTA → CalBooking → VoiceChat modal.
- Head schema per page: BreadcrumbList, LocalBusiness (geo + areaServed City sameAs + containedInPlace + makesOffer + speakable), Service (provider.knowsAbout + areaServed sameAs/containedInPlace), ImageObject (contentLocation), FAQPage (matches the page's 5 Q&As verbatim).
...
## Wiring checklist when adding a town (do all 6)
